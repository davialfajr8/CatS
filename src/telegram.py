...
def resend_message(chat_id, ev, reply_to=None, force_caption: FormattedMessage=None):
	...
	if ev.content_type == "text":
		return bot.send_message(chat_id, f"{ev.from_user.first_name}: {ev.text}", **kwargs)
	elif ev.content_type == "photo":
...
@communonebot merupakan bot grup anonim yang dibuat khusus untuk chat hanya dengan voice note/audio.

✅ HANYA PESAN SUARA YANG DAPAT TERKIRIM 🎙✨

✅ TEKS HANYA UNTUK TOPIK (ONLY ADMIN) 📄🎯

⚠️ Hati-hati ya! Kalau kirim pesan suara sembarangan, ngomong kasar, bikin pernyataan ofensif, hate speech, konten NSFW, spam, atau iklan, kamu bakal langsung kena BANNED! 🚫👮‍♂️

📌 Grup utama: @momentouscloud.

"""
	if user.rank == 0:
		if ev.content_type != "voice":
			bot.send_message(ev.from_user.id, PESAN.format(nama=ev.from_user.first_name))
			return
